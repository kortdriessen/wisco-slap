...
def make_activity_movie(
    ...
    act_time: np.ndarray | None = None,  # (n_timepoints,), seconds
    soma_map: np.ndarray | None = None,  # (H, W), 0 or [1..n_somas]
    soma_activity: np.ndarray | None = None,  # (n_somas, n_timepoints)
    soma_time: np.ndarray | None = None,  # (n_timepoints,), seconds
    # Timeline / resampling
    fps: float | None = None,  # if None -> derive from act_time (or 1 frame/sample)
    start_time: float | None = None,  # seconds; if None -> overlap intersection start
    end_time: float | None = None,  # seconds; if None -> overlap intersection end
    ...
    output_size: tuple[int, int] | None = None,  # (H_out, W_out); if None -> native
    ...
    crf: int | None = 18,  # for libx264; ignored by nvenc (use 'cq' below)
    ...
    nvenc_cq: int | None = None,  # e.g. 19; used when codec == "h264_nvenc"
    gop: int | None = None,  # e.g. fps*2; None keeps default
    tune: str | None = None,  # e.g. "animation" for libx264
    video_options: dict[str, str]
    | None = None,  # raw ffmpeg options if you need extra control
    # UX
    progress: bool = True,
    ...
    def _resample_to_frame_indices(
        t_sig: np.ndarray | None, n_t: int, frame_times: np.ndarray
    ) -> np.ndarray:
        """
...
def generate_activity_colorbar_image(
    ...
    syn_color: str | tuple[int, int, int] = "green",
    ...
    # Soma mapping
    soma_color: str | tuple[int, int, int] = "red",
    soma_threshold: float = 0.15,
    soma_saturate_at: float = 0.8,
    soma_alpha_max: float = 0.95,
    soma_alpha_gamma: float = 1.0,
    ...
    axis_color: str | tuple[int, int, int] = "white",
...
def animate_state_bar_video(
    ...
    state_colors: dict[str, str | tuple[int, int, int]],
    fps: float = 60.0,
    # Appearance
    width: int = 1280,
    height: int = 200,
    ...
    for fi in range(frames_total):
        ...
        if progress and (fi + 1) % max(1, int(fps)) == 0:
            print(
                f"[animate_state_bar_video] encoded {fi + 1}/{frames_total} frames ..."
            )
...
def animate_traces_video(
    out_path: str,
    traces: np.ndarray | list[np.ndarray],  # (T,), or list of (T,) or (N,T)
    time: np.ndarray,  # (T,), seconds; strictly increasing
    fps: float = 60.0,
    ...
    ylim: tuple[float, float] | None = None,  # override if desired
    ylims: list[tuple[float, float]]
    | None = None,  # per-trace limits for stacked layout
    # Colors & styling
    trace_colors: str
    | tuple[int, int, int]
    | list[str | tuple[int, int, int]] = "cyan",
    ...
    grid: bool = False,
    # Labels
    trace_labels: str | list[str] | None = None,
    # State bar
    states: list[str]
    | None = None,  # len T strings labeling each sample (e.g. "Wake","NREM")
    state_colors: dict[str, str | tuple[int, int, int]] | None = None,
    ...
    crf: int | None = 18,  # libx264
    preset: str = "veryfast",  # libx264 preset
    nvenc_cq: int | None = None,  # e.g. 19, if using h264_nvenc
    pix_fmt: str = "yuv420p",
    pad_to_even: bool = True,
    progress: bool = True,
):
...
def generate_peripheral_trace_movie(
    ...
    ep_data_ds, time_array = epy.sigpro.gen.downsample_signal(
        epslice_data, fs, 200.0, t=ep_slice_time
    )
